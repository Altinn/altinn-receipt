export const en: Record<string, Record<string, string>> = {
  receipt_platform: {
    attachments: 'Attachments',
    date_sent: 'Date sent',
    date_archived: 'Date archived',
    helper_text: 'A mechanical check has been completed while filling in, but we reserve the right to detect errors during the processing of the case and that other documentation may be necessary. Please provide the reference number in case of any inquiries to the agency.',
    helper_text_a2lookup: 'The information that was collected from the public sector is saved and signed in Altinn. Click the link/attachment to view the information in a separate window.',
    is_sent: 'is submitted',
    receipt: 'Receipt',
    receiver: 'Receiver',
    reference_number: 'Reference number',
    sender: 'Sender',
    sent_content: 'The following is submitted:',
    log_out: 'Log out',
    profile_icon_aria_label: 'Profile icon button',
    back_to_inbox: 'Back to inbox',
  },
};
